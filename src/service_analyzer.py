...
class ServiceAnalyzer:
    ...
    def analyze_services(self, port_results):
        ...
        self.logger.info(f"[+] Analyzing services for {len(port_results)} hosts")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for (hostname, port, service_type), future in future_to_host.items():
                try:
                    ...
                    if self.verbose:
                        self.logger.debug(f"[-] Error analyzing {service_type} on {hostname}:{port} - {e}")
    ...
    def _analyze_web_service(self, hostname, port):
        ...
        try:
            ...
            if self.verbose:
                self.logger.debug(f"[+] Analyzed web service at {base_url} - {result['title'] or 'No title'}")
                
        except Exception as e:
            if self.verbose:
                self.logger.debug(f"[-] Error accessing {base_url}: {e}")
        ...
        for path in self.common_web_paths:
            try:
                ...
                if response.status_code != 404:
                    ...
                    if self.verbose:
                        self.logger.debug(f"[+] Found interesting path: {path_url} - Status: {response.status_code}")
            except:
                pass
        
        return result# service_analyzer.py - Analyze detected services
