...
class Enumerator:
    def __init__(self, shodan_client=None, quiet=False, verbose=False):
    ...
    def enumerate_targets(self, domains):
        ...
        self.logger.info(f"[+] Starting subdomain enumeration for {len(domains)} domain(s)")
        ...
        for domain in domains:
            ...
            self.logger.info(f"[+] Found {len(self.subdomains)} subdomains for {domain}")
        ...
        try:
            ...
            for ns in nameservers:
                try:
                    ...
                    if self.verbose:
                        self.logger.debug(f"[-] Zone transfer failed for {domain} from {ns}")
    ...
    def _enumerate_certificate_transparency(self, domain):
        ...
        try:
            ...
            self.logger.warning(f"[-] Error retrieving certificate transparency data for {domain}: {e}")
    ...
    def _enumerate_shodan(self, domain):
        ...
        try:
            ...
            if self.verbose:
                self.logger.debug(f"[-] Error retrieving Shodan data for {domain}: {e}")
    ...
    def _resolve_ips(self):
        ...
        self.logger.info(f"[+] Resolving IP addresses for {len(self.subdomains)} subdomains")
        
        # Only show progress bar if not in quiet mode
        if self.quiet:
            # Without progress bar for quiet mode
            ...
            # With progress bar for normal mode
            for subdomain in tqdm(self.subdomains):
                if subdomain not in self.ip_mapping:
                    try:
                        ...
                        if self.verbose:
                            self.logger.debug(f"[-] Failed to resolve {subdomain}: {e}")
                        pass  # Failed to resolve# enumerator.py - Subdomain enumeration functionality
