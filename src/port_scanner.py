...
class PortScanner:
    ...
    def scan_targets(self, ip_mapping):
        ...
        self.logger.info(f"[+] Starting port scan on {len(ip_mapping)} targets")
        ...
        self.logger.info(f"[+] Port scan complete. Found open ports on {len(results)} targets")
        return host_results
    ...
    def scan_single_target(self, ip):
        ...
        try:
            ...
            arguments = '-T4 -sV'
            if not self.verbose:
                arguments += ' -q'  # Quiet mode for nmap itself
                
            self.nmap_scanner.scan(ip, ports_str, arguments=arguments)
            ...
            if ip in self.nmap_scanner.all_hosts():
                ...
                for proto in self.nmap_scanner[ip].all_protocols():
                    ...
                    for port in ports:
                        ...
                        if self.verbose:
                            self.logger.debug(f"[+] {ip}:{port} - {service_info['name']} {service_info.get('product', '')} {service_info.get('version', '')}")
                            
                return result
            return None
        except Exception as e:
            if self.verbose:
                self.logger.debug(f"[-] Error during port scan of {ip}: {e}")
            return None
    ...
    def quick_scan(self, ip, custom_ports=None):
        ...
        try:
            ...
            if self.verbose:
                self.logger.debug(f"[-] Error during quick scan of {ip}: {e}")
            return None# port_scanner.py - Scan for open ports on target IPs
